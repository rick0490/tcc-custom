 ...
 * @param {Response} res - the result
 */
// Cache-busting timestamp - set once at server start
...
function getHtml (req, res) {
	...
	// Add cache-busting to all JS and CSS files to prevent CDN caching
	const cacheBuster = `?v=${serverStartTime}`;
	html = html.replace(/src="([^"]+\.js)"/g, `src="$1${cacheBuster}"`);
	html = html.replace(/href="([^"]+\.css)"/g, `href="$1${cacheBuster}"`);

	// Also inject the cache buster for the module loader to use
	html = html.replace('window.mmVersion', `window.mmCacheBuster = ${serverStartTime};\n      window.mmVersion`);
