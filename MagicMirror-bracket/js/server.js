...
function Server (config) {
	...
	this.open = function () {
		return new Promise((resolve) => {
			...
			let directories = ["/config", "/css", "/flyers", "/modules", "/node_modules/animate.css", "/node_modules/@fontsource", "/node_modules/@fortawesome", "/translations", "/tests/configs", "/tests/mocks"];
			...
			for (const directory of uniqDirs) {
				app.use(directory, express.static(path.resolve(global.root_path + directory), {
					setHeaders: (res, filepath) => {
						// Disable caching for module JavaScript files to ensure updates are loaded
						if (filepath.includes('/modules/') && filepath.endsWith('.js')) {
							res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate');
							res.setHeader('Pragma', 'no-cache');
							res.setHeader('Expires', '0');
						}
						// Disable caching for config files to ensure config updates are loaded
