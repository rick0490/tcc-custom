...
router.post('/register', asyncHandler(async (req, res) => {
	...
	if (!hostname || !mac) {
		throw new ValidationError('Hostname and MAC address are required');
	}
...
router.post('/:id/heartbeat', asyncHandler(async (req, res) => {
	...
	if (!display) {
		throw new NotFoundError('Display', id);
	}
	...
	if (previousView !== 'match' && currentView === 'match' && matchPolling && matchPolling.fetchAndPushMatches) {
		logger.log('matchRefresh', { displayId: id, hostname: display.hostname });
		matchPolling.fetchAndPushMatches().catch(err => {
			logger.error('matchRefresh', err, { displayId: id });
		});
	}
...
// ============================================
router.get('/flyer/settings', asyncHandler(async (req, res) => {
...
// Requires authentication
// ============================================
router.put('/flyer/settings', asyncHandler(async (req, res) => {
	const userId = req.session.userId;
	if (!userId) {
		throw new UnauthorizedError('Not authenticated');
	}
	...
	res.json({ success: true, message: 'Settings updated', requestId: req.requestId });
}));
...
// ============================================
router.post('/flyer/control', asyncHandler(async (req, res) => {
	const userId = req.session.userId;
	if (!userId) {
		throw new UnauthorizedError('Not authenticated');
	}
	...
	res.json({ success: true, action, requestId: req.requestId });
}));
...
// Requires authentication
// ============================================
router.post('/flyer/volume', asyncHandler(async (req, res) => {
	const userId = req.session.userId;
	if (!userId) {
		throw new UnauthorizedError('Not authenticated');
	}
	...
	res.json({ success: true, volume, requestId: req.requestId });
}));
...
// Requires authentication
// ============================================
router.get('/flyer/playlist', asyncHandler(async (req, res) => {
	const userId = req.session.userId;
	if (!userId) {
		throw new UnauthorizedError('Not authenticated');
	}
...
// Requires authentication
// ============================================
router.put('/flyer/playlist', asyncHandler(async (req, res) => {
	const userId = req.session.userId;
	if (!userId) {
		throw new UnauthorizedError('Not authenticated');
	}
	...
	if (items) {
		for (const item of items) {
			if (!item.filename) {
				throw new ValidationError('Each item must have a filename');
			}
	...
	res.json({ success: true, message: 'Playlist updated', requestId: req.requestId });
}));
...
// ============================================
router.post('/flyer/playlist/control', asyncHandler(async (req, res) => {
	const userId = req.session.userId;
	if (!userId) {
		throw new UnauthorizedError('Not authenticated');
	}
	...
	if (action === 'next') {
		...
		return res.json({ success: true, action: 'toggle', enabled: !!enabled, requestId: req.requestId });
	}
	...
	res.json({ success: true, action, currentIndex, requestId: req.requestId });
}));
...
// ============================================
router.post('/flyer/status', asyncHandler(async (req, res) => {
...
router.get('/:id/config', asyncHandler(async (req, res) => {
	...
	if (display.pendingCommand) {
		...
		saveDisplays(displaysData);
		logger.log('pendingCommand', { displayId: id, action: pendingCommand.action });
	}
		...
		if (timeSinceHeartbeat > 90000) {
			...
			if (previousStatus === 'online') {
				...
				if (pushNotifications && pushNotifications.broadcastPushNotification && display.userId) {
					...
					}, display.userId).catch(err => logger.error('pushDisplayOffline', err, { displayId: display.id }));
				}
			}
		}
	});
...
router.put('/:id/config', asyncHandler(async (req, res) => {
	...
	if (!assignedView && displayScaleFactor === undefined) {
		throw new ValidationError('At least one configuration field is required (assignedView or displayScaleFactor)');
	}

	if (displayScaleFactor !== undefined) {
		const scale = parseFloat(displayScaleFactor);
		if (isNaN(scale) || scale < 0.5 || scale > 3.0) {
			throw new ValidationError('Display scale factor must be between 0.5 and 3.0');
		}
	}
	...
	if (ownership.error) {
		if (ownership.status === 404) throw new NotFoundError('Display', id);
		if (ownership.status === 403) throw new ForbiddenError('Access denied');
	}
...
// Multi-tenant: Verifies display belongs to authenticated user
// ============================================
router.post('/:id/reboot', asyncHandler(async (req, res) => {
	const { id } = req.params;
	...
	logger.log('reboot', { displayId: id, hostname: display.hostname });
...
// Multi-tenant: Verifies display belongs to authenticated user
// ============================================
router.post('/:id/shutdown', asyncHandler(async (req, res) => {
	const { id } = req.params;
	...
	logger.log('shutdown', { displayId: id, hostname: display.hostname });
...
// Multi-tenant: Verifies display belongs to authenticated user
// ============================================
router.post('/:id/debug', asyncHandler(async (req, res) => {
	const { id } = req.params;
	...
	logger.log('debugMode', { displayId: id, hostname: display.hostname, enabled });
...
router.post('/:id/logs', asyncHandler(async (req, res) => {
	...
	if (!logs || !Array.isArray(logs)) {
		throw new ValidationError('Logs array is required');
	}
...
// Multi-tenant: Verifies display belongs to authenticated user
// ============================================
router.delete('/:id/logs', asyncHandler(async (req, res) => {
	const { id } = req.params;
