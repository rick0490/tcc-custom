/**
 * WebSocket Client for Match Display
 *
...
class WebSocketClient {
    constructor(adminWsUrl, userId) {
        ...
        this.eventHandlers = {};
        this.displayId = `web-match-${userId}-${Date.now()}`;
        this.lastMatchHash = null;
    ...
    setupEventListeners() {
        ...
        this.socket.on('connect', () => {
            ...
            // Register as display with user context
            console.log('[WebSocket] Registering as display...');
        ...
        this.eventTypes.forEach(event => {
            this.socket.on(event, (data) => {
                if (event === 'matches:update') {
                    console.log('[WebSocket] Received matches:update, matches:', data.matches?.length || 0);
                }
    ...
    handleEvent(event, data) {
        // Special handling for matches:update with hash deduplication
        if (event === 'matches:update' && data.updateHash) {
            if (data.updateHash === this.lastMatchHash) {
                this.log('skippingDuplicate', { hash: data.updateHash });
                return;
            }
            this.lastMatchHash = data.updateHash;

            // Send acknowledgment
            this.socket.emit('matches:ack', {
                displayId: this.displayId,
